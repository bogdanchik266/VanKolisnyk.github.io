export const header = {
    ...
    methods:{
        toogleActive() {
            if (this.active == 1) {
                this.active = 0;
                document.body.classList.remove('menu-active'); // Приховуємо сірий фон
            } else {
                this.active = 1;
                document.body.classList.add('menu-active'); // Додаємо сірий фон
            }
        }
    ...
    <div class="wrapper">
        <div class="flex">
            ...
            <div class="w10 al" id="user-top" v-if="parent.user && parent.user.user">
                ...
                <!-- Бургер-меню (три смужки) -->
                <i class="fas fa-bars" @click="menu=1"></i>
                <!-- Меню, яке відкривається -->
                <div id="user-info" :class="{active:active==1}">
                 <a class="#" @click.prevent="parent.logout();"><li class="fas fa-sign-out-alt"></li> {{parent.user.user}} Log out</a>
                </div>
            </div>
        </div>
    </div>
